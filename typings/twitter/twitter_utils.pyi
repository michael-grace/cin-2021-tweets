...
URL_REGEXP = re.compile(r'(' r'^(?!(https?://|www\.)?\.|ftps?://|([0-9]+\.){{1,3}}\d+)' r'(?:https?://|www\.)*^(?!.*@)(?:[\w+-_]+[.])' r'(?:{0}\b' r'(?:[:0-9]))' r'(?:[\w+\/]?[a-z0-9!\*\'\(\);:&=\+\$/%#\[\]\-_\.,~?])*' r')'.format(r'\b|'.join(TLDS)), re.U | re.I | re.X)
def calc_expected_status_length(status, short_url_length=...):
...
def parse_media_file(passed_media, async_upload=...):
